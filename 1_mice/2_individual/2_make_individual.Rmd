...
The goal of this script is to generate a Seurat object from the raw count matrix for sample `r params$sample_name`.
...
if (!dir.exists(paste0(out_dir, "/datasets/"))) {
  dir.create(paste0(out_dir, "/datasets/"), recursive = TRUE)
}
```
...
```{r cell_markers}
cell_markers = readRDS(paste0(out_dir, "/../1_metadata/mice_cell_markers.rds"))
...
```{r dotplot_markers}
dotplot_markers = readRDS(paste0(out_dir, "/../1_metadata/mice_dotplot_markers.rds"))
dotplot_markers
```
...
* `Ensembl_ID` : Ensembl ID, as stored in the `features.tsv.gz` file
* `gene_name` : gene name, as stored in the `features.tsv.gz` file. Duplicated gene names will have the same name.
...
We add the sample columns as in metadata :
...
```{r plot_piechart_df, class.source = "fold-hide"}
...
piechart_all_cells = plot_piechart(df = sobj@meta.data,
...
piechart_doublets = plot_piechart(df = sobj@meta.data %>%
...
piechart_percent.mt = plot_piechart(df = sobj@meta.data %>%
...
piechart_percent.rb = plot_piechart(df = sobj@meta.data %>%
...
piechart_log_nCount_RNA = plot_piechart(df = sobj@meta.data %>%
...
piechart_nFeature_RNA = plot_piechart(df = sobj@meta.data %>%
