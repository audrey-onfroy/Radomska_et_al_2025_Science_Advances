...
```{r out_dir}
out_dir = "/home/aurelien/Documents/Audrey/git_analysis/mpnst_project/2_individual_datasets/"
```

We load the parameters :

```{r get_param}
sample_name = params$sample_name # "2022_23"
# sample_name = "2022_23"
```
...
```{r cell_markers}
cell_markers = readRDS(paste0(out_dir, "/../1_metadata/datasets_cell_markers.rds"))
...
```{r dotplot_markers}
dotplot_markers = readRDS(paste0(out_dir, "/../1_metadata/datasets_dotplot_markers.rds"))
dotplot_markers
```
...
```{r sample_info}
sample_info = readRDS(paste0(out_dir, "/../1_metadata/datasets_sample_info.rds"))
sample_info %>%
...
We set an upper bound to 20,000 cells. Remaining cells are out of cells RNA embed in droplets :
...
saveRDS(sobj, paste0(out_dir, "/", sample_name, "_sobj_unfiltered.rds"))
```
...
saveRDS(sobj, file = paste0(out_dir, "/", sample_name, "_sobj_filtered.rds"))
```
