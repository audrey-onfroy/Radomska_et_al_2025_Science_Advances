...
We make a highly resolutive clustering to set a root cluster in pNF sample for trajectory :

```{r clustering}
sobj = Seurat::FindNeighbors(sobj, reduction = "mnn_dm", dims = c(1:7))
...
We set the root in the cluster expressing highest level of Pde4b gene.

```{r Pde4b_avg}
root_cluster = data.frame(Pde4b_level = Seurat::FetchData(sobj, vars = "Pde4b", slot = "data")[, 1],
                          clusters = sobj$seurat_clusters) %>%
  dplyr::group_by(clusters) %>%
  dplyr::summarise(Pde4b_avg = mean(Pde4b_level)) %>%
  dplyr::filter(Pde4b_avg == max(Pde4b_avg)) %>%
  dplyr::pull(clusters) %>%
  as.character()
...
We visualize root cluster and Pde4b expression levels :

```{r see_root, fig.width = 12, fig.height = 5}
gene_expr = Seurat::FeaturePlot(sobj, features = "Pde4b", reduction = traj_umap) +
...
We perform the trajectory inference, from a pNF sample :
...
We visualize retained pseudotime :
