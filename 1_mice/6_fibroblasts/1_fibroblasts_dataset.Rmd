...
This script is used to make a fibroblasts dataset from fl/fl samples.
...
To locate fibroblasts, we look at Ccl11 expression levels, cell type annotation, and clustering as information.
...
plot_list = lapply(sobj_list, FUN = function(one_sobj) {
  ...
  plot_sublist[[1]] = Seurat::FeaturePlot(one_sobj, reduction = name2D,
                                          features = "Ccl11", order = TRUE) +
    ggplot2::labs(title = project_name,
                  subtitle = "Ccl11 expression") +
    Seurat::NoAxes() +
  ...
  plot_sublist[[4]] = Seurat::DimPlot(one_sobj,
    ...
    ggplot2::labs(title = "Cluster annotation",
                  subtitle = paste0(sum(one_sobj$cluster_type == "fibroblasts"),
                                    " fibroblasts")) +
    Seurat::NoAxes() + Seurat::NoLegend() +
    ggplot2::theme(aspect.ratio = 1,
                   plot.title = element_text(hjust = 0.5),
...
sobj_list = lapply(sobj_list, FUN = function(one_sobj) {
  one_sobj$is_of_interest = (one_sobj$cluster_type == "fibroblasts")
...
sobj_list = sobj_list[!is.na(sobj_list)]
...
For each sample, we remove cells not enough genes :

```{r subset_mito}
sobj_list = lapply(sobj_list, FUN = function(one_sobj) {
  one_sobj$is_of_interest = (one_sobj$nFeature_RNA > nFeature_RNA_thresh)
...
Which are the remaining datasets ?

```{r sub_project_names_oi}
project_names_oi = names(sobj_list)
```
...
We drop empty levels in factors :
...
sobj$mnn_clusters = sobj$seurat_clusters
```
...
We visualize samples on the diffusion map UMAP :

```{r dm_umap_visu, fig.width = 5, fig.height = 4}
...
We remove sample-specific effect with harmony :
...
```{r clustering_harmony}
sobj = Seurat::FindNeighbors(sobj, reduction = reduction_name)
sobj = Seurat::FindClusters(sobj, resolution = 1)
sobj$harmony_clusters = sobj$seurat_clusters
...
```{r see_harmony_umap_tsne, fig.width = 8, fig.height = 4}
tsne = Seurat::DimPlot(sobj, group.by = "project_name",
                       reduction = paste0(reduction_name, "_", ndims, "_tsne")) +
  ggplot2::scale_color_manual(values = sample_info$color,
                              breaks = sample_info$project_name) +
  Seurat::NoAxes() + ggplot2::ggtitle("harmony - tSNE") +
  ggplot2::theme(aspect.ratio = 1,
                 plot.title = element_text(hjust = 0.5),
                 legend.position = "none")
...
umap = Seurat::DimPlot(sobj, group.by = "project_name",
  ...
  ggplot2::scale_color_manual(values = sample_info$color,
                              breaks = sample_info$project_name) +
  Seurat::NoAxes() + ggplot2::ggtitle("harmony - UMAP") +
  ggplot2::theme(aspect.ratio = 1,
                 plot.title = element_text(hjust = 0.5))
...
```{r split_by_genotype_harmony, fig.width = 10, fig.height = 3}
plot_list = aquarius::plot_split_dimred(sobj,
                                        reduction = paste0(reduction_name, "_", ndims, "_umap"),
                                        split_by = "tumor_type",
                                        split_color = rep("black", 3),
...
We keep this representation :

```{r set_name2D}
name2D = "mnn_44_umap"

sobj$seurat_clusters = sobj$mnn_clusters
Seurat::Idents(sobj) = sobj$mnn_clusters
```
...
```{r fibro_split_by_origin, fig.width = 12, fig.height = 9}
sobj$all_cells = TRUE
