...
This script is used to generate a tumor cells dataset from each individual dataset.
...
```{r gene_expression, class.source = "fold-hide", fig.width = 12, fig.height = 6}
...
```{r table_orig_ident}
table(sobj$project_name, sobj$genotype)
```
...
column_of_interest = c("orig.ident", "sample_type", "project_name",
                       "genotype", "condition", "sample_identifier",
...
We make a representation split by genotype :

```{r split_by_genotype, fig.width = 10, fig.height = 5}
...
```{r save_harmony, fig.width = 12, fig.height = 12}
